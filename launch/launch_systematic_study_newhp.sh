...
POSTFIX=${COMMNAME}_R${IMPSAMPR}_N${BUFFSIZE}_D${DKLPARAM}_B${BATCHNUM}_O${EPERSTEP}_L${NETLRATE}_S${LAYRSIZE}_TRIAL${RUNTRIAL}
NMASTERS=1
echo $POSTFIX

source launchDaint_openai.sh humanw_${POSTFIX} Humanoid-v2    settings/settings_bench_args_hp.sh
source launchDaint_openai.sh spider_${POSTFIX} Ant-v2         settings/settings_bench_args_hp.sh
source launchDaint_openai.sh walker_${POSTFIX} Walker2d-v2    settings/settings_bench_args_hp.sh
source launchDaint_openai.sh cheeta_${POSTFIX} HalfCheetah-v2 settings/settings_bench_args_hp.sh
