...
int main()
{
	...
	const double uNot[3] = {4,0,0};
	const double tStart = 0, tEnd = 10;
	const int N = 100000;
	...
	forceX = new double[N](); forceY = new double[N](); torqueExt = new double[N]();
	...
	for (int i=0; i<N; i++){
		...
		thrustL[i] = cos(tt[i]*angFactor/4.0);
		thrustR[i] = 0.0;
		torque[i] = (0.5*params.l)*(thrustR[i] - thrustL[i]) + torqueExt[i];
		fXext[i] = 0.0;//*cos(tt[i]*angFactor/4.0);
		fYext[i] = dist(gen)*sin(tt[i]*angFactor);
	...
	for(int i=0; i<N; i++)	fprintf(temp, "%f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f\n", tt[i], forceX[i], forceY[i], u[i], v[i], r[i], x[i], y[i], theta[i]);
	fclose(temp);
