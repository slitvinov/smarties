...
def computeAverages(scalars):
    ...
    epsv = [scalars[i]['eps']          for i in range(len(scalars))]
    ...
    return [np.mean(tkes), np.mean(lmbd), np.mean(rels),    \
            np.mean(tint), np.mean(lint), np.mean(grad), np.mean(epsv)],   \
           [ np.std(tkes),  np.std(lmbd),  np.std(rels),    \
             np.std(tint),  np.std(lint),  np.std(grad),  np.std(epsv)],
...
def main_integral(path):
    ...
    dataM, dataV, inps = np.zeros([0, 7]), np.zeros([0, 7]), np.zeros([0, 3])
    ...
    for ei in range(len(EPSs)):
      for ni in range(len(NUs)):
        for li in range(len(EXTs)):
          ...
          print("eps %f nu %f N:%d" % (EPSs[ei],NUs[ni],len(allScalars)))
          means, stdevs = computeAverages(allScalars)
          dataM = np.append(dataM, np.asarray(means ).reshape([1,7]), 0)
          dataV = np.append(dataV, np.asarray(stdevs).reshape([1,7]), 0)
    ...
    funs += [ fitFun ]
    popt += [ fitFunction(inps, dataM, dataV, 6, funs[-1] ) ] # fitLint
    print('visc eps fit:', popt[-1])

    if True:
      plt.figure()
      axes = [ plt.subplot(1,7,1), plt.subplot(1,7,2), plt.subplot(1,7,3), \
               plt.subplot(1,7,4), plt.subplot(1,7,5), plt.subplot(1,7,6), plt.subplot(1,7,7) ]
      ...
      axes[6].set_ylabel('Viscous Dissipation')
      for ax in axes     : ax.grid()
      #for ax in axes[:3] : ax.set_xticklabels([])
      #for ax in axes[3:] : 
      ax.set_xlabel('Energy Injection Rate')
...
def main_spectral(path):
    ...
    # k, eps, leta, lint, nu = x
    #for i in range(kdata.shape[0]-1, -1, -1):
    #  if kdata[i,0] * kdata[i,3] < 2:
    #    eksigma = np.delete(eksigma, i, 0)
    #    ekdata = np.delete(ekdata, i, 0)
    #    kdata = np.delete(kdata, i, 0)
    ...
    for i in range(allSpectra.shape[0]):
      ...
      Yfit = np.array([EkFunc([k,eps,leta,lint,nu], popt[0],popt[1],popt[2],popt[3]) for k in X])
