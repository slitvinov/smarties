...
if __name__ == '__main__':
  ...
  epIDs = np.linspace(2000, len(terminals)-1000, len(colors), dtype=int)
  for epid in range(len(colors)): #-1, len(colors) ): #-1, -1, -1):
    ind = epIDs[epid]
    print(ind)
    ...
    s = np.asarray(s)
    x = np.asarray(x)
    alpha = 0.1 + 0.6*epid/len(colors)
    print(epid)
    print(alpha)
    #plt.fill_between(x,-y-s,-y+s, facecolor=colors[-epid], alpha=alpha, zorder = epid)
    plt.plot(x, -y, 'o-', color=colors[-epid]) #, zorder= - len(colors) - epid)
