...
class DACER : public Learner_offPolicy
{
 protected:
  ...
  {
    const Real V_cur = outVec[VsID], rho = pol_cur.sampRhoWeight;
    //after this call delta_ret is updated and can be used for pol and val grad:
    const Real dAdv = updateVret(S, t, V_cur, pol_cur); // (updates S->Q_RET[t])

    const Rvec policyG = pol_cur.policy_grad(pol_cur.sampAct, S->Q_RET[t]*rho);
    ...
    gradient[VsID] = beta*alpha * std::min((Real)1, rho) * S->Q_RET[t];
    pol_cur.finalize_grad(finalG, gradient);

    Vstats[thrID].dumpStats(V_cur, gradient[VsID]); //bookkeeping
  ...
  inline Real updateVret(Sequence*const S, const Uint t, const Real V,
    const Policy_t& pol) const {
      // update also KL div from behav. pol for current time step
  ...
  inline Real updateVret(Sequence*const S, const Uint t, const Real V,
    ...
    const Real delta = (rNext +gamma*(W*V_RET + vNext) -V);
    S->setStateValue(t, V ); S->setRetrace(t, delta);
