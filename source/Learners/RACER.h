...
class RACER : public Learner_utils
{
	...
	std::vector<std::mt19937>& generators;
		...
		const vector<Uint> net_outputs = {1, nL, nA, nA};
		const vector<Uint> net_indices = {0,  1, 1+nL, 1+nL+nA};
	#elif defined ACER_SAFE // output V(s), P(s), pol(s), mu(s)
		const vector<Uint> net_outputs = {1, nL, nA, nA};
		const vector<Uint> net_indices = {0,  1, 1+nL, 1+nL+nA};
	#else // output V(s), P(s), pol(s), prec(s), mu(s)
		const vector<Uint> net_outputs = {1, nL, nA, nA, nA};
		const vector<Uint> net_indices = {0,  1, 1+nL, 1+nL+nA, 1+nL+2*nA};
	#endif
			...
			const Gaussian_policy*const pol) const
	{
		...
		#else
		return Quadratic_advantage(net_indices[1],net_indices.back(),nA,nL,out,pol);
		#endif
	}
