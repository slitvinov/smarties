...
#define _allocateClean(name, size) { const int nsimd = __vec_width__/sizeof(Real); const int sizeSIMD=std::ceil(size/(Real)nsimd)*nsimd; posix_memalign((void **)& name, __vec_width__, sizeSIMD); memset(name, 0, sizeSIMD); }
#define _allocateQuick(name, size) { const int nsimd = __vec_width__/sizeof(Real); const int sizeSIMD=std::ceil(size/(Real)nsimd)*nsimd; posix_memalign((void **)& name, __vec_width__, sizeSIMD); }
