...
WaveletNet::WaveletNet(vector<int>& layerSize, Real eta, Real alpha, int batchSize) :
...
void WaveletNet::predict(const vector<Real>& inputs, vector<Real>& outputs, int nAgent)
{
    Real res = 0;
...
void WaveletNet::improve(const vector<Real>& inputs, const vector<Real>& errors, int nAgent)
{
    // Save all the info about current object
    
    vector<Real> tmpVec(1);
...
void WaveletNetLM::improve(const vector<Real>& inputs, const vector<Real>& errors, int nAgent)
{
    // Save all the info about current object
    ...
    if (nInBatch == batchSize)
    {
        ...
        while (Q > Q0)
        {
            ...
            for (int i=0; i<batchSize; i++)
            {
                ...
                Real diff = tmpVec[0] - batchExact[i];
                Q += diff * diff;
            }
