...
public:
	...
	NormalLink(int nI, int iI, int nO, int iO, int iW, int nO_simd) :
		iW(iW), nI(nI), iI(iI), nO(nO), iO(iO), nW(nI*nO_simd), nO_simd(nO_simd)
    {
...
public:
    ...
    void save(std::ostringstream & o, Real* const _weights) const override
    {
        o << std::setprecision(10);
        for (int i=0; i<nI; i++) for (int o=0; o<nO; o++)
        	o << _weights[iW  + nO_simd*i + o];

        for (int i=0; i<nI; i++) for (int o=0; o<nO; o++)
        	o << _weights[iWI + nO_simd*i + o];

        for (int i=0; i<nI; i++) for (int o=0; o<nO; o++)
        	o << _weights[iWF + nO_simd*i + o];

        for (int i=0; i<nI; i++) for (int o=0; o<nO; o++)
        	o << _weights[iWO + nO_simd*i + o];
    }
...
public:
	...
	iW(iW), nI(nI), iI(iI), nO(nO), iO(iO), outputDepth(nO_simd), inputWidth(inW), inputHeight(inH), inputDepth(inD), filterWidth(fW), filterHeight(fH),
    ...
    void save(std::ostringstream & o, Real* const _weights) const override
    {
        ...
        for (int i = 0; i < nAdded; i++)
		for (int o = 0; o < nO; o++)
			o << _weights[iW + outputDepth_simd*i + o];
    }
...
public:
	...
	void initialize(mt19937* const gen, Real* const _weights) const override
    {
        for (int p=0 ; p<2; p++)
        for (int o=0 ; o<nO; o++)
        	_weights[iW +p*nO_simd+ o] = Real(1==p); //set to 1 the scaling factor
    }
...
struct Graph //misleading, this is just the graph for a single layer
{
	...
	layerSize(-1), layerSize_simd(-1), firstNeuron_ID(-1), firstState_ID(-1), firstBias_ID(-1), firstBiasWhiten(-1),
    ...
    void initializeWeights(mt19937* const gen, Real* const _weights, Real* const _biases) const
    {
        ...
        assert(layerSize>0 && layerSize_simd>0 && firstNeuron_ID>=0 && firstBias_ID>=0);
