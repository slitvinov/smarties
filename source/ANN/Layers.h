...
public:
                       ...
                       Grads* const grad, const Real* const weights, const Real* const biases) const  override
    {
        ...
        const Real* __restrict__ const gradbias = grad->_B +n1stBias;
...
public:
                       ...
                       Grads* const grad, const Real* const weights, const Real* const biases) const  override
    {
        ...
        const Real* __restrict__ const gradbiasC = grad->_B +n1stBias;
        const Real* __restrict__ const gradbiasI = grad->_B +n1stBiasIG;
        const Real* __restrict__ const gradbiasF = grad->_B +n1stBiasFG;
        const Real* __restrict__ const gradbiasO = grad->_B +n1stBiasOG;
        __builtin_assume_aligned(outputI,  __vec_width__);
        __builtin_assume_aligned(outputF, __vec_width__);
        __builtin_assume_aligned(outputO, __vec_width__);
        __builtin_assume_aligned(outputC,  __vec_width__);
...
public:
                       ...
                       Grads* const grad, const Real* const weights, const Real* const biases) const  override
    {
        ...
        const Real* __restrict__ const link_inputs = curr->outvals +link->iI;
        const Real* __restrict__ const link_means = biases + n1stBias;
        const Real* __restrict__ const link_vars = biases + n1stBias +nNeurons;
        const Real* __restrict__ const link_shifts = weights + link->iW;
        const Real* __restrict__ const link_scales = weights + link->iW +nNeurons;
        Real* __restrict__ const grad_means = grad->_B + n1stBias;
        Real* __restrict__ const grad_vars = grad->_B + n1stBias +nNeurons;
        Real* __restrict__ const grad_scales = grad->_W + link->iW;
        Real* __restrict__ const grad_shifts = grad->_W + link->iW +nNeurons;
        __builtin_assume_aligned(errors,  __vec_width__);
        __builtin_assume_aligned(inputs, __vec_width__);
        ...
        for (int n=0; n<nNeurons; n++)  {
            ...
            grad_means[n] += 0.01*dEdMean;
            grad_vars[n] += 0.01*(dEdMean*dEdMean - link_vars[n]);
#endif
