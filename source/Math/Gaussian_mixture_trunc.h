...
private:
  ...
  {
    const long double top = -std::pow( act -mean +2*NORMDIST_MAX, 2) *prec /2;
    const long double mid = -std::pow( act -mean                , 2) *prec /2;
    const long double bot = -std::pow( act -mean -2*NORMDIST_MAX, 2) *prec /2;
    // policy is truncated as shown in function sample()
    return std::sqrt(prec/M_PI/2)*(std::exp(top)+std::exp(mid)+std::exp(bot));
  }
  ...
  inline Rvec policy_grad(const Rvec& act, const Real factor) const
  {
    ...
    for(Uint j=0; j<nExperts; j++) {
      ...
      for (Uint i=0; i<nA; i++) {
        ...
        const long double mult = -std::pow(ut, 2) *precisions[j][i] /2;
        const long double mulm = -std::pow(um, 2) *precisions[j][i] /2;
        const long double mulb = -std::pow(ub, 2) *precisions[j][i] /2;
        ...
        ret[indM] = fac*precisions[j][i]*(ut*Pt + um*Pm + ub*Pb)/(Pt+Pm+Pb);
        const auto precFac = (ut*ut*Pt + um*um*Pm + ub*ub*Pb)/(Pt+Pm+Pb);
        ret[indS] = fac*(precFac*precisions[j][i]-1)/stdevs[j][i];
      }
    }
    return ret;
  }
