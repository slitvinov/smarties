...
Network::Network(vector<int>& layerSize, double eta, double alpha, int batchSize) :
nInputs(layerSize.front()), nOutputs(layerSize.back()), nLayers(layerSize.size()), eta(eta), alpha(alpha), batchSize(batchSize), rng(0)
{
	...
	{
		Layer* l = (i == nLayers - 1) ? new Layer(layerSize[i],  new Tanh) : new Layer(layerSize[i],  new Tanh);
...
Network(layerSize, 0, 0, batchSize), muFactor(muFactor)
{
	mu = 0.01;
	muMax = 1e10;
	muMin = 1e-3;
