...
void MemoryBlock::exec()
{
    for (int i=0; i<nMemoryCells; i++)
    {
        ...
        for (int j=0; j<mCells[i]->ScO->outLinks.size(); j++)
            mCells[i]->ScO->outLinks[j]->val = mCells[i]->Sc_new;
    }
...
{ // Heuristics galore
    if (hasInputs)
    {
        ...
        bool bEligibTrace(false);
        bool bAdaptiveLearnR(false); //not working
        bool bRprop(false);
        bool bMix(false); //HAX!
        bool bExponentialAdapt(true);
        for (int i=0; i<inLinks.size(); i++)
        {
            ...
            if (bExponentialAdapt)
            {
                inLinks[i]->m = 0.8;
                inLinks[i]->q = 1e-6;
                
                inLinks[i]->a_dw2 = inLinks[i]->m*inLinks[i]->a_dw2 + (1.-inLinks[i]->m)*inLinks[i]->Dw*inLinks[i]->Dw;
                //cout << inLinks[i]->Dw * inLinks[i]->a_dw/inLinks[i]->a_dw2 << endl;
                if (inLinks[i]->a_dw2>1e-9)
                inLinks[i]->mu = inLinks[i]->mu * max(1e-3, 1. + inLinks[i]->q *(inLinks[i]->Dw * inLinks[i]->a_dw/inLinks[i]->a_dw2));
                inLinks[i]->a_dw = inLinks[i]->m*inLinks[i]->a_dw + (1.-inLinks[i]->m)*inLinks[i]->Dw;
                inLinks[i]->mu = min(inLinks[i]->mu, 10.);
                //cout << inLinks[i]->mu << endl;
                inLinks[i]->Dw *= inLinks[i]->mu;
            }
            ...
            if (bRprop)
            {
                ...
                {
                    inLinks[i]->Delta = min(1.2*inLinks[i]->Delta, fabs(inLinks[i]->w));
                ...
                if (dEdw * inLinks[i]->o_dEdw < 0.)
                {
                    inLinks[i]->Delta = max(0.5*inLinks[i]->Delta, lambda*fabs(inLinks[i]->w));
                ...
                {
                    inLinks[i]->Dw = Delta_w;
                    bMomentumLearn = false;
                }
            }
            */
            if (fabs(inLinks[i]->Dw)>fabs(inLinks[i]->w))
            {
                if (inLinks[i]->Dw>0.)
                    inLinks[i]->Dw =  fabs(inLinks[i]->w);
                if (inLinks[i]->Dw<0.)
                    inLinks[i]->Dw = -fabs(inLinks[i]->w);
            }
            ...
            if (bMomentumLearn && alpha>0.)
            {
                ...
                if (inLinks[i]->factor>10.) inLinks[i]->factor = 10.;
                //printf("%f\n",inLinks[i]->factor);
            }
...
nInputs(layerSize.front()), nOutputs(layerSize.back()), nLayers(layerSize.size()), eta(eta), alpha(alpha),  rng(0), nAgents(nAgents), nMems(0), nRecurr(layerSize.front()), kappa(kappa), olderr(1)
{
    ...
    for (int i=1; i<nLayers-1; i++)
    {
        ...
        for (int j=0; j<i+1; j++)
        {
            debug4("- connecting back to layer %d\n", j);
            HiddenLayer* prev = layers[j];
            hl->connect2layers(prev, &rng, i-j-1);
        }
        
        debug4("- connecting to ground signal\n");
    ...
    debug4("Creating last layer with %d neurons\n", nOutputs);
    ...
    debug4("- connecting last layer to outputs and errors\n");
